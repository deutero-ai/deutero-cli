...
func newWebhooksDeleteCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "Permanently removes the endpoint and its delivery log.",
		Example:     "  deutero-pp-cli webhooks delete 00000000-0000-4000-8000-000000000004",
		Annotations: map[string]string{"pp:endpoint": "webhooks.delete", "pp:method": "DELETE", "pp:path": "/api/v1/webhooks/{webhook_id}", "pp:happy-args": "webhook_id=00000000-0000-4000-8000-000000000004"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"webhook_id": true})
				}
				if len(filtered) > 0 {
					var parsed any
			...
			printErr := printOutputWithFlagsMeta(cmd.OutOrStdout(), data, flags, map[string]any{"source": "live"}, map[string]bool{"webhook_id": true})
			if partialFailure != nil && !flags.allowPartialFailure {
				return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "webhooks", partialFailure.Message))
			}
			return printErr
