...
type rootFlags struct {
	...
	// invocation. Mirrors the DEUTERO_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "deutero-pp-cli",
		Short: `Deutero CLI — Every Deutero endpoint, plus the flow-behaviour and fielding answers that take many API calls to assemble and that no…`,
		Long: `Deutero CLI — Every Deutero endpoint, plus the flow-behaviour and fielding answers that take many API calls to assemble and that no single endpoint returns.

Highlights (not in the official API docs):
  • flow coverage   Shows how an Interview Flow actually routed across a whole study: every branch arm's share, how often the otherwise path fired, classifier errors, which steps interviews reached, and which declared v…
  ...
  • study diff   Compares two complete study definitions field by field — details, welcome and translations, screening, characteristics, questions or flow, recruitment — or compares a live study against a saved bundl…
  • answers matrix   Turns a study into a real table: one row per interview, one column per question or captured variable, with the external participant id as the join key and optional participant characteristic columns …
  • flow effects   Aggregates what a flow's side steps actually did across a study: failed data fetches with status codes, undelivered signals, and dry-run signals from simulations kept separate from real ones. Flow-mo…
  • study bundle   Snapshots an entire study definition — details, welcome and translations, screening, characteristics, questions or flow, recruitment — into one file. The write half is a separate command, 'study appl…
  • rehearse diff   Compares the simulated persona cohort against real participants on the same measures: step reach, branch arm mix, variable capture rate, message count and duration. Needs both cohorts; branch and var…
...
Health check: run 'deutero-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("deutero-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select characteristics,completed,embed_metadata)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'deutero-pp-cli profile list')")
	...
	rootCmd.AddCommand(newEmbedCmd(flags))
	rootCmd.AddCommand(newProjectsCmd(flags))
	rootCmd.AddCommand(newSimulationsCmd(flags))
	...
	rootCmd.AddCommand(newCreditsPromotedCmd(flags))
	rootCmd.AddCommand(newGraphPromotedCmd(flags))
	rootCmd.AddCommand(newHealthPromotedCmd(flags))
	rootCmd.AddCommand(newInterviewsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAnswersCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCrosstabCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFieldingCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFlowCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelReconcileCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRehearseCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSaturationCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelStudyCmd(flags))
