...
import (
	...
	"github.com/deutero-ai/deutero-cli/internal/learn/lookups"
...
// pathContextVarSet is the --path-context typo-warning set. Tenant vars are
// also listed in endpointTemplateVarSet so the unresolved-key skip ignores them.
var pathContextVarSet = map[string]bool{
	"study_id": true,
}

var syncPathContextValues = map[string]string{}
...
Resource scoping:
  ...
  deutero-pp-cli sync --resources webhooks,embed

  # Full resync (ignore previous checkpoint)
  deutero-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  deutero-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  deutero-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  deutero-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			c.NoCache = true
			// Tenant vars go through Config.TemplateVars / buildURL. Ordinary
			// SkipDefaultSync path params are applied later via replacePathParam.
			if len(pathContext) > 0 {
				// Warn on keys outside pathContextVarSet so typos
				...
				for k := range pathContext {
					if !pathContextVarSet[k] {
			...
			syncPathContextValues = pathContext
...
func syncResource(ctx context.Context, c interface {
	...
	for name, value := range syncPathContextValues {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "interviews":
		return db.UpsertInterviews(data)
	case "fetches_and_signals":
		return db.UpsertFetchesAndSignals(data)
	case "transcript":
		return db.UpsertTranscript(data)
	case "projects":
		return db.UpsertProjects(data)
	case "projects_studies":
		return db.UpsertProjectsStudies(data)
	case "simulations":
		return db.UpsertSimulations(data)
	case "studies":
		return db.UpsertStudies(data)
	case "analysis":
		return db.UpsertAnalysis(data)
	case "characteristics":
		return db.UpsertCharacteristics(data)
	case "studies_embed":
		return db.UpsertStudiesEmbed(data)
	case "studies_graph":
		return db.UpsertStudiesGraph(data)
	case "studies_interviews":
		return db.UpsertStudiesInterviews(data)
	case "personas":
		return db.UpsertPersonas(data)
	case "questions":
		return db.UpsertQuestions(data)
	case "recruitment":
		return db.UpsertRecruitment(data)
	case "screening":
		return db.UpsertScreening(data)
	case "search":
		return db.UpsertSearch(data)
	case "studies_simulations":
		return db.UpsertStudiesSimulations(data)
	case "stats":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"embed":                "/api/v1/embed/keys",
		"graph":                "/api/v1/graph/node-types",
		"health":               "/health",
		"interviews":           "/api/v1/studies/{study_id}/interviews",
		"projects":             "/api/v1/projects",
		"simulations":          "/api/v1/studies/{study_id}/simulations",
		"transcripts":          "/api/v1/studies/{study_id}/transcripts",
		"webhooks":             "/api/v1/webhooks",
		"webhooks-event-types": "/api/v1/webhooks/event-types",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "deliveries", ParentTable: "webhooks", ParentIDParam: "webhook_id", PathTemplate: "/api/v1/webhooks/{webhook_id}/deliveries", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.webhook", PathParams: []dependentPathParamDef{
			{Param: "webhook_id", Field: "id"},
		}},
	}
}
...
var flatReconcileModes = map[string]string{
	"interviews":           "flat_global",
	"projects":             "flat_global",
	"simulations":          "flat_global",
	"transcripts":          "flat_global",
	"webhooks-event-types": "flat_global",
}
