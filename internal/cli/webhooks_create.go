...
func newWebhooksCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create",
		Short:       "Subscribe a URL of yours to organization events.",
		Example:     "  deutero-pp-cli webhooks create --label \"Example webhook\" --url https://example.com/deutero-hook",
		Annotations: map[string]string{"pp:endpoint": "webhooks.create", "pp:method": "POST", "pp:path": "/api/v1/webhooks", "pp:requires-input": "true"},
	...
	cmd.Flags().BoolVar(&bodyEnabled, "enabled", true, "Whether deliveries start immediately")
	...
	cmd.Flags().StringVar(&bodyLabel, "label", "", "How you will recognize this endpoint in the list")
	cmd.Flags().StringVar(&bodyUrl, "url", "", "Publicly reachable http(s) URL that receives the POSTs")
