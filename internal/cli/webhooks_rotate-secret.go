...
func newWebhooksRotateSecretCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "rotate-secret",
		Short:       "Create rotate secret for webhooks",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newWebhooksRotateSecretRotateWebhookSecretCmd(flags))
	return cmd
}
