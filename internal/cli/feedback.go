...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DEUTERO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DEUTERO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/deutero-ai/deutero-cli/feedback")
...
When ` + "`DEUTERO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DEUTERO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  deutero-pp-cli feedback "docs said exclusive but the boundary is inclusive"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  deutero-pp-cli feedback list
  deutero-pp-cli feedback list --limit 5
  deutero-pp-cli feedback list --json`,
