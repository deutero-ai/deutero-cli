...
func jwtExpirySource(cfg *config.Config) string {
	...
	if v := strings.TrimSpace(cfg.DeuteroApiKey); v != "" {
		return v
	}
...
func classifyHTMLTransportError(err error) error {
	...
	if htmlLooksLikeAuthFailure(evidence) {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set it with: echo \"$TOKEN\" | deutero-pp-cli auth set-token or export DEUTERO_API_KEY=\"your-token-here\""))
	}
...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'deutero-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Set it with: echo \"$TOKEN\" | deutero-pp-cli auth set-token or export DEUTERO_API_KEY=\"your-token-here\""+
			"\n      Run 'deutero-pp-cli doctor' to check auth status.", err))
