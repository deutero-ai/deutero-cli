...
In local mode: searches locally synced data only.`,
  ...
  deutero-pp-cli search "error timeout"

  # Force local search only
  deutero-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  deutero-pp-cli search "status" --type credits --data-source local
  # JSON output for piping
  deutero-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v1/studies/{study_id}/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v1/studies/{study_id}/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'deutero-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
