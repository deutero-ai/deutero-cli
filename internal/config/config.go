...
type Config struct {
	...
	DeuteroApiKey    string          `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://dashboard.deutero.ai/study-api",
	}
	...
	// Env var overrides
	if v := cliutil.EnvOverride("DEUTERO_API_KEY"); v != "" {
		cfg.DeuteroApiKey = v
		cfg.markEnvOverride("DeuteroApiKey")
		cfg.AuthSource = "env:DEUTERO_API_KEY"
		cfg.CredentialSource = "env:DEUTERO_API_KEY"
	}
	...
	if v := cliutil.EnvOverride("DEUTERO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("DEUTERO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "deutero-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.DeuteroApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:DEUTERO_API_KEY"
		}
		return "Bearer " + c.DeuteroApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.DeuteroApiKey != "" {
		parts = append(parts, "api_key="+c.DeuteroApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.DeuteroApiKey == "" {
		c.DeuteroApiKey = creds.DeuteroApiKey
	}
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["DeuteroApiKey"] {
			out.DeuteroApiKey = c.fileConfig.DeuteroApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DeuteroApiKey":
		c.fileConfig.DeuteroApiKey = c.DeuteroApiKey
	}
}
